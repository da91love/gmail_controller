...
class AccessService():
    ...
    def select_is_in_process_history(self, **bindings):
        ...
        try:
            return AccessServiceBase(self.db).execute_sql(
                sql=Query.sql_select_is_in_process_history,
                bindings=bindings)

        except Exception as e:
            raise e

    def insert_op_process(self, **bindings):
        """
        :param bindings: (tuple)
        :return: (list) sql query result
        """
        try:
            return AccessServiceBase(self.db).execute_sql(
                sql=Query.sql_insert_op_process,
                bindings=bindings)

        except Exception as e:
            raise e

    def truncate_invoice_master(self, **bindings):
        """
        :param bindings: (tuple)
        :return: (list) sql query result
        """
        try:
            return AccessServiceBase(self.db).execute_sql(
                sql=Query.sql_truncate_invoice_master,
                bindings=bindings)
