...
def api_tiktok_kpi_checker(event, context=None):
    ...
    date_object = datetime.strptime('2024-07-23', '%Y-%m-%d').date()
    
    for i in range(0, 30):
        # today_as_min = datetime.combine(datetime.now(), time.min)
        tg_date = datetime.combine(date_object + timedelta(days=i), time.min)
        tg_date_as_min = datetime.combine(tg_date, time.min)

        from_date = DateUtil.get_previous_day(tg_date=tg_date_as_min, tg_day='tuesday')
        to_date = tg_date_as_min + timedelta(days=1)
